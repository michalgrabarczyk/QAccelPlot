...
//
#include "materials/PointCloudMaterial.hpp"
...
static_assert(sizeof(PointCloudUboTail) == 28);
static_assert(kPointCloudUboSize == 144);
...
public:
    PointCloudShader()
    {
        setShaderFileName(VertexStage, QString::fromLatin1(":/qaccelplot/src/shaders/point_cloud.vert.qsb"));
        setShaderFileName(FragmentStage, QString::fromLatin1(":/qaccelplot/src/shaders/point_cloud.frag.qsb"));
    }
    ...
    bool updateUniformData(RenderState& state, QSGMaterial* newMaterial, QSGMaterial* oldMaterial) override
    {
        ...
        if (!DataTextureMaterial::writeCommonUniforms(buffer->data(), buffer->size(), state.combinedMatrix(), material)) {
            return false;
        }
