...
public:
    ...
    /// \brief Rebuilds the index from double coordinates without narrowing precision.
    void buildDouble(const double* data, int itemCount, int valuesPerItem = 4);
...
private:
    ...
    template <typename T> void buildData(const T* data, int itemCount, int valuesPerItem);
