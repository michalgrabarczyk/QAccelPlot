...
coordinates, or small differences at a large offset. These paths retain the
input as doubles and rebase the GPU buffer to keep nearby values distinct. The
APIs whose names end in `F` intentionally store floats and should be used only
when the coordinate values are representable at the required precision.
...
- For the `F` ingestion paths, store points as interleaved floats:
  `[x0, y0, x1, y1, ...]`.
